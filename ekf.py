...
om_var = 10.5  # rotational velocity variance
r_var = 0.00003  # range measurements variance
b_var = 0.00009  # bearing measurement variance
...
def measurement_update(lk, rk, bk, P_check, x_check):
    ...
    H = np.array([[-A/dist, -B/dist, (d[0]*(C*np.sin(x_check[2])+D*np.cos(x_check[2])))/dist],
                  [B/(dist**2), -A/(dist**2), (D*(d[0]*np.sin(x_check[2])+-D)+C*(d[0]*np.cos(x_check[2])-C))/(dist**2)]])
    M = np.eye(2)
    R = cov_y
...
for k in range(1, len(t)):  # start at 1 because we've set the initial prediction
    ...
    F_km = np.array([[1, 0, -delta_t*v[k-1]*np.sin(x_check[2])], [0, 1, delta_t*v[k-1]*np.cos(x_check[2])], [0, 0, 1]])
